...
function ZoomInitSearch()
{
	...
	for (sw = 0; sw < NumSearchWords; sw++) {
		...
		for (kw_ptr = 0; kw_ptr < dictwords.length; kw_ptr++) {
		    ...
		    //Mod Start
			var urltest = "file:///E:/Folder1kjhkjhkjhk";
			var check = 1;
			var strlength;
			if (checkbox1)
			{
			    strlength = "file:///E:/Folder1"; 
			    var strpassed = urltest.substr(0, strlength.length);
			    check = strpassed.localeCompare(strlength);
			}

			if (checkbox2)
			{
			    strlength = "file:///E:/Folder2";
			    var strpassed = urltest.substr(0, strlength.length);
			    check = strpassed.localeCompare(strlength);
			}

			if (match_result != -1 && check == 1)
			{//Mod End
...
function ZoomShowMatchOptions()
{
	...
	txt.appendChild(document.createTextNode('Folder1'));
	...
	txt2.appendChild(document.createTextNode('Folder2'));
...
function ZoomShowResults()
{
	...
	while (arrayline < matches && arrayline < result_limit) {
		...
		//    check = strpassed.localeCompare(strlength);
		//}
	    ////Mod End except for if statement

	    ////MOd
		//var pststr = "pst";
		//var msgstr = "msg";

		//for (kz = 0; kz < pgurl.length - 3; kz++) {
		//    var substring = pgurl.substring(kz, kz + 3);

		//    if (substring.localeCompare(msgstr) == 0) {
		//        pgurl = pgurl.substr(0, kz + 3);
		//    }
		//    if (substring.localeCompare(pststr) == 0) {
		//        pgurl = pgurl.substr(0, kz + 3);
		//    }
		//}
		urlLink = pgurl;
...
function showwiki(){
	document.writeln("<div class = \"blah\"> TITLE")
	document.writeln("<div class = \"yada\">" + getParam("zoom_query"));
...
{
	document.writeln("<div class = \"yada\">" + getParam("zoom_query"))
	if(getParam("zoom_query") == "")
		document.writeln("<div class = \"yada\"> INSIDE");
